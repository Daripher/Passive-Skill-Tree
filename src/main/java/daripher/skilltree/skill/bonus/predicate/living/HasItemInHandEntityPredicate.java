...
public final class HasItemInHandEntityPredicate implements LivingEntityPredicate {
  ...
  public boolean test(LivingEntity living) {
    return itemStackPredicate.test(living.getMainHandItem())
           || itemStackPredicate.test(living.getOffhandItem());
  }
  ...
  public static class Serializer implements LivingEntityPredicate.Serializer {
    ...
    public LivingEntityPredicate deserialize(JsonObject json) throws JsonParseException {
      return new HasItemInHandEntityPredicate(SerializationHelper.deserializeItemCondition(json));
    }

    @Override
    public void serialize(JsonObject json, LivingEntityPredicate condition) {
      if (!(condition instanceof HasItemInHandEntityPredicate aCondition)) {
    ...
    public LivingEntityPredicate deserialize(CompoundTag tag) {
      return new HasItemInHandEntityPredicate(SerializationHelper.deserializeItemCondition(tag));
    }
    ...
    public LivingEntityPredicate deserialize(FriendlyByteBuf buf) {
      return new HasItemInHandEntityPredicate(NetworkHelper.readItemCondition(buf));
    }
