...
/**
 * Class to serialize and deserialize {@link Attribute} and {@link SlotAttributeWrapper} objects in
 * JSON format.
 */
public class AttributeSerializer implements JsonSerializer<Attribute>, JsonDeserializer<Attribute> {
  ...
  public Attribute deserialize(JsonElement json, Type typeOfT, JsonDeserializationContext context)
    ...
    else attribute = ForgeRegistries.ATTRIBUTES.getValue(id);
    if (attribute == null) throw new NullPointerException("Attribute " + id + " doesn't exist!");
    return attribute;
  }
  ...
  public JsonElement serialize(Attribute src, Type typeOfSrc, JsonSerializationContext context) {
    ...
    else id = ForgeRegistries.ATTRIBUTES.getKey(src);
