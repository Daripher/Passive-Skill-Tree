...
public abstract class MixinEnchantmentMenu implements EnchantmentMenuExtention {
	...
	private int reduceLevelRequirements(Level level, BlockPos pos, int slot, int power, ItemStack itemStack, int enchantmentLevel) {
		...
		Optional<Player> player = ContainerHelper.getViewingPlayer((EnchantmentMenu) (Object) this);
		if (!player.isPresent()) return levelRequirement;
		return EnchantmentHelper.adjustLevelRequirement(levelRequirement, player.get());
	}

	@Redirect(method = { "lambda$slotsChanged$0",
		"m_39483_" }, at = @At(value = "INVOKE", target = "Lnet/minecraft/world/inventory/EnchantmentMenu;getEnchantmentList(Lnet/minecraft/world/item/ItemStack;II)Ljava/util/List;"))
	...
	@Redirect(method = { "lambda$clickMenuButton$1",
		"m_39475_" }, at = @At(value = "INVOKE", target = "Lnet/minecraft/world/inventory/EnchantmentMenu;getEnchantmentList(Lnet/minecraft/world/item/ItemStack;II)Ljava/util/List;"))
