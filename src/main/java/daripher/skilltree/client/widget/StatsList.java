...
public class StatsList extends AbstractWidget {
	...
	public void renderButton(PoseStack poseStack, int mouseX, int mouseY, float partialTick) {
		...
		for (int i = scroll; i < maxLines + scroll; i++) {
			drawString(poseStack, font, stats.get(i), x + 5, y + 5 + (i - scroll) * (font.lineHeight + 3), 0x7B7BE5);
		}
		if (stats.size() > maxLines) {
			...
			fill(poseStack, x + width - 3, y + scrollShift, x + width, y + scrollShift + scrollSize, 0xDD888888);
		}
	}
