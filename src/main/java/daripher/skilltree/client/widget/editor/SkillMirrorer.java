...
public class SkillMirrorer extends AbstractWidget {
    ...
    public void init() {
        ...
        editor.addNumericTextField(160, 0, 40, 14, mirrorSides).setNumericFilter(v -> v > 1)
                .setNumericResponder(v -> mirrorSides = v.intValue());
        editor.increaseHeight(19);
        editor.addLabel(0, 0, "Angle", ChatFormatting.GOLD);
        editor.addNumericTextField(160, 0, 40, 14, mirrorAngle).setNumericResponder(v -> mirrorAngle = v.floatValue());
        editor.increaseHeight(19);
        editor.addLabel(0, 0, "Center", ChatFormatting.GOLD);
        editor.addNumericTextField(160, 0, 40, 14, mirrorCenterX).setNumericResponder(v -> mirrorCenterX = v.floatValue());
        editor.addNumericTextField(115, 0, 40, 14, mirrorCenterY).setNumericResponder(v -> mirrorCenterY = v.floatValue());
    ...
    public @Nullable PassiveSkill getMirroredSkill(PassiveSkill skill, int sector) {
        ...
        float originalAngle = (float) Math.toDegrees(Math.atan2(skillY - mirrorCenterY, skillX - mirrorCenterX)) + 90;
        float sectorSize = 360f / mirrorSides;
        float angle = (float) Math.toRadians(mirrorSides == 2 ? -originalAngle + mirrorAngle * 2 : originalAngle + sectorSize * sector);
