...
public class GemLootHandler {
  ...
  public static void dropGemFromOre(BlockEvent.BreakEvent event) {
    ...
    LootContext lootContext = createGemsLootContext(event, serverLevel, player);
    float multiplier =
        1f + SkillBonusHandler.getLootMultiplier(player, LootDuplicationBonus.LootType.GEMS);
    while (multiplier > 1) {
      dropGems(serverLevel, event.getPos(), lootTable, lootContext);
      multiplier--;
    }
  ...
  private static void dropGems(
      Level level, BlockPos blockPos, LootTable lootTable, LootContext lootContext) {
